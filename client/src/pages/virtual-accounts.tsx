...
export default function VirtualAccountsPage() {
  ...
  return <div className="min-h-screen bg-slate-50 pb-28 md:pb-8">
    <WavyHeader title="Virtual Currency Accounts" subtitle="Apply for USD, GBP or EUR account details after review" onBack={() => setLocation("/dashboard")} size="lg" icon={<Building2 className="w-6 h-6" />} />

    <main className="max-w-3xl mx-auto px-4 -mt-6 space-y-4">
      <div className="grid grid-cols-3 gap-2">
        {Object.entries(currencyMeta).map(([code, meta]) => <button key={code} onClick={() => meta.enabled && setCurrency(code)} disabled={!meta.enabled} className={`rounded-2xl border p-3 text-left ${!meta.enabled ? "opacity-60 cursor-not-allowed" : "hover:shadow"}`}>
      ...
      {selectedApplication?.status === "approved" && selectedApplication.accountDetails ? <Card className="border-emerald-200">
        ...
        <CardContent className="space-y-3">
          { ["accountName","bankName","accountNumber","routingNumber","sortCode","iban","swiftCode","bankAddress","beneficiaryAddress"].map(k => selectedApplication.accountDetails?.[k] && <div key={k} className="flex items-center justify-between"><div className="text-sm text-slate-700">{k.replace(/([A-Z])/g, ' $1')}</div><div className="font-mono text-sm">{selectedApplication.accountDetails?.[k]}</div></div>) }
      ...
      </Card> : selectedApplication ? <Card><CardContent className="py-8 text-center"><Clock className="w-10 h-10 text-amber-500 mx-auto mb-3" /><h2 className="font-bold capitalize">{selectedApplication.status}</h2><p className="text-sm text-slate-500 mt-2">Your application is currently under review by our compliance team.</p></CardContent></Card> : <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {step === 1 ? <div className="space-y-3">
            <div><Label>Source of income</Label><Input value={form.sourceOfIncome} onChange={e => { setFormError(""); setForm({ ...form, sourceOfIncome: e.target.value }); }} placeholder="Employment, business, investments" /></div>
            <div><Label>Expected monthly volume</Label><Input value={form.monthlyVolume} onChange={e => { setFormError(""); setForm({ ...form, monthlyVolume: e.target.value }); }} placeholder="e.g. 1,000 - 5,000" /></div>
            <div><Label>Purpose of account</Label><Textarea value={form.purpose} onChange={e => { setFormError(""); setForm({ ...form, purpose: e.target.value }); }} placeholder="Receiving salary, client payments, remittances" /></div>
            ...
            {[
              ["notUsCitizen", "I confirm I am not a US citizen, resident, or tax person."], ["notPoliticallyExposed", "I am not politically exposed and am not related to a politically exposed person."], ["beneficialOwner", "I am the ultimate beneficial owner of the funds."], ["truthfulInformation", "I confirm that the information I provided is true and accurate."], ["acceptsTerms", "I accept the terms and conditions."]
            ].map(([key, label]) => <label key={key} className="flex gap-3 rounded-xl border p-3 text-sm"><Checkbox checked={(declarations as any)[key]} onCheckedChange={v => setDeclarations({ ...declarations, [(key as string)]: !!v })} /> <span>{label}</span></label>)}
            <div className="rounded-xl bg-emerald-50 p-3 text-sm text-emerald-800 flex gap-2"><ShieldCheck className="w-4 h-4 mt-0.5" /> Approved users see one admin-configured account per currency after compliance review.</div>
          </div>}
          ...
          <div className="sticky bottom-24 z-10 -mx-2 flex gap-2 rounded-2xl border bg-white/95 p-2 shadow-lg backdrop-blur md:static md:mx-0 md:border-0 md:bg-transparent md:p-0 md:shadow-none">
            ...
            <div className="flex-1 text-right">
              {step === 1 ? <Button disabled={!canContinue} onClick={() => setStep(2)}>Continue</Button> : <Button disabled={!canContinue || applyMutation.isLoading} onClick={() => applyMutation.mutate()}>Submit application</Button>}
            </div>
          </div>
        </CardContent>
      </Card>}
