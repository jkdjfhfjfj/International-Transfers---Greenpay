...
const COMING_SOON = [
  { icon: Briefcase,   label: 'Business',       description: 'Business accounts & payroll',        color: 'from-blue-500 to-indigo-600' },
  { icon: PiggyBank,   label: 'Savings',         description: 'Goal-based savings plans',           color: 'from-pink-500 to-rose-600' },
...
function RatesPanel() {
  ...
  const displayCurrencies = Object.keys(CURRENCY_META).filter(code => rates[code] !== undefined);
  ...
  return (
    <div>
      ...
      {isLoading ? (
        ...
        <p className="text-xs text-center text-gray-400 py-4">Rates unavailable — check API key in settings</p>
      ) : (
        <div className="grid grid-cols-2 gap-2">
          {displayCurrencies.map(code => {
            const meta = CURRENCY_META[code];
            return (
              <div key={code} className="flex items-center gap-2 bg-gray-50 dark:bg-gray-700/60 rounded-xl px-3 py-2.5">
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed bottom-[4.5rem] md:bottom-0 left-0 right-0 z-[10001] bg-white dark:bg-gray-900 rounded-t-3xl shadow-2xl flex flex-col"
          ...
          >
            ...
            <div className="flex gap-2 px-5 pb-3 flex-shrink-0">
              {(['rates', 'features'] as const).map(tab => (
                <button
                  key={tab}
                ...
                >
                  {tab === 'rates' ? <TrendingUp className="w-3.5 h-3.5" /> : <Grid className="w-3.5 h-3.5" />}
                  {tab === 'rates' ? 'Live Rates' : 'Coming Soon'}
                </button>
              ))}
            </div>
