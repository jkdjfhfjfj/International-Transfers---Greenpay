...
__export(schema_exports, {
  ...
  insertVirtualAccountSchema: () => insertVirtualAccountSchema,
...
var init_schema = __esm({
  "shared/schema.ts"() {
    ...
    announcementDismissals = pgTable("announcement_dismissals", {
      userId: varchar("user_id").references(() => users.id, { onDelete: "cascade" }).notNull(),
    ...
    virtualAccounts = pgTable("virtual_accounts", {
      id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
      userId: varchar("user_id").references(() => users.id, { onDelete: "cascade" }).notNull(),
    ...
    insertVirtualAccountSchema = createInsertSchema(virtualAccounts).omit({ id: true, createdAt: true, updatedAt: true });
    insertLedgerEntrySchema = createInsertSchema(ledgerEntries).omit({ id: true, createdAt: true });
...
var init_storage = __esm({
  "server/storage.ts"() {
    ...
    MemStorage = class {
      ...
      withdrawalEvents = /* @__PURE__ */ new Map();
      ...
      async getPaymentRequestsReceivedByUser(userId, email, phone) {
        return Array.from(this.paymentRequests.values()).filter(
          (req) => req.toUserId === userId || !!email && req.toEmail?.toLowerCase() === email.toLowerCase() || !!phone && req.toPhone === phone
    ...
    DatabaseStorage = class {
      ...
      async getPaymentRequestsReceivedByUser(userId, email, phone) {
        ...
        return await db.select().from(paymentRequests).where(or(...conditions)).orderBy(desc(paymentRequests.createdAt));
      }
      async updatePaymentRequest(id, updates) {
...
var NEXUSPAY_BASE_URL = "https://makamescopay.com/api";
...
var NexusPayService = class {
  ...
  async getStatus(reference) {
    ...
    const response = await fetch2(`${this.baseUrl}/payments/${reference}`, {
      headers: this.headers(apiKey)
    });
    const data = await response.json();
...
async function getConfiguredFeeRate(key, fallbackPercent) {
  try {
...
async function getTransactionFeeRate() {
...
async function verifyTransactionSecurity(userId, credentials) {
  const user = await storage.getUser(userId);
  ...
  const pinEnabled = Boolean(user.pinEnabled && user.pinCode);
  ...
  if (!hasPin && !hasAuthenticator) {
    return {
      ok: false,
      status: 400,
      requiresPin: methods.pin && pinEnabled,
      requiresAuthenticator: methods.authenticator && authenticatorEnabled,
      securityOptions: { pin: methods.pin && pinEnabled, authenticator: methods.authenticator && authenticatorEnabled },
      message: "PIN or authenticator code required"
    };
  }
  let valid = false;
  if (hasPin && pinEnabled) valid = await bcrypt2.compare(credentials.pin, user.pinCode);
  if (!valid && hasAuthenticator && authenticatorEnabled) {
    valid = twoFactorService.verifyToken(user.twoFactorSecret, credentials.authenticatorCode);
  }
  if (!valid) return { ok: false, status: 401, message: "Invalid PIN or authenticator code" };
  return { ok: true };
}
...
async function registerRoutes(app2) {
  ...
  app2.get("/api/virtual-accounts", requireAuth, async (req, res) => {
    try {
      ...
      const accounts = await db.select().from(virtualAccounts).where(eq3(virtualAccounts.userId, userId));
      ...
      res.json({
        ...
        applications: applications.map((app3) => ({
          ...
          virtualAccount: app3.status === "approved" ? (() => {
            const account = accounts.find((a) => a.applicationId === app3.id);
            return account ? {
              ...account,
  ...
  app2.put("/api/admin/virtual-accounts/currencies", requireAdminAuth, async (req, res) => {
    try {
      const currencies = z.array(z.string()).transform(
        (values) => Array.from(new Set(values.map(normalizeCurrency).filter((currency) => allVirtualAccountCurrencies.includes(currency))))
  ...
  app2.put("/api/admin/virtual-accounts/:id/hold", requireAdminAuth, async (req, res) => {
    try {
      ...
      res.status(500).json({ message: error?.message || "Failed to update virtual-account hold" });
    }
  });
  app2.put("/api/admin/virtual-accounts/:id/balance", requireAdminAuth, async (req, res) => {
    try {
      const amount = Number(req.body.amount);
      ...
      const account = await db.query.virtualAccounts.findFirst({ where: eq3(virtualAccounts.id, req.params.id) });
  ...
  app2.post("/api/virtual-accounts/:id/transfer", requireAuth, async (req, res) => {
    try {
      ...
      const account = await db.query.virtualAccounts.findFirst({ where: and2(eq3(virtualAccounts.id, req.params.id), eq3(virtualAccounts.userId, userId)) });
      ...
      const debit = await applyLedgerEntry({
        ...
        currency: account.currency,
        amount: -amount,
        entryType: "virtual_account_transfer_out",
      ...
      try {
        const credit = await applyLedgerEntry({
          ...
          amount,
          entryType: "virtual_account_transfer_in",
        ...
        await applyLedgerEntry({
          ...
          currency: account.currency,
          amount,
          entryType: "virtual_account_transfer_rollback",
...
(function(){
    ...
    try {
      ...
      try {
        airtimeDebit = await applyLedgerEntry({
          ...
          entryType: "airtime",
          idempotencyKey: `airtime:${userId}:${Date.now()}`,
      ...
      try {
        ...
        await applyLedgerEntry({
          ...
          currency: "KES",
          amount: purchaseAmount,
          entryType: "airtime_rollback",
          idempotencyKey: `airtime-rollback:${userId}:${Date.now()}`,
    ...
    try {
      ...
      try {
        await applyLedgerEntry({
          ...
          idempotencyKey: `bill:${billPayment.id}`,
          description: `Bill payment - ${provider}`
        });
      } catch (error) {
        return res.status(400).json({ message: error?.message || "Insufficient KES balance" });
    ...
    try {
      ...
      try {
        ...
        await applyLedgerEntry({
          ...
          entryType: "exchange",
          idempotencyKey: `exchange:${exchangeReference}:credit`,
  ...
  app2.post("/api/payment-requests", requireAuth, async (req, res) => {
    try {
      const sessionUserId = req.session?.userId;
      if (!sessionUserId) {
        return res.status(401).json({ message: "Authentication required" });
      }
      const requestedRecipient = req.body?.toUserId ? await storage.getUser(String(req.body.toUserId)) : void 0;
      const requestData = insertPaymentRequestSchema.parse({
        ...req.body,
        fromUserId: sessionUserId,
        toUserId: req.body?.toUserId || void 0,
        toEmail: req.body?.toEmail || requestedRecipient?.email || void 0,
        toPhone: req.body?.toPhone || requestedRecipient?.phone || void 0,
      ...
      if (requestData.toUserId || requestData.toEmail || requestData.toPhone) {
        const recipientUser = requestData.toUserId ? await storage.getUser(requestData.toUserId) : requestData.toEmail ? await storage.getUserByEmail(requestData.toEmail) : requestData.toPhone ? await storage.getUserByPhone(requestData.toPhone) : void 0;
        if (recipientUser) {
          await notificationService.sendNotification({
  ...
  app2.put("/api/payment-requests/:id/:action", requireAuth, async (req, res) => {
    try {
      ...
      if (action === "cancel") {
        ...
        const updated2 = await storage.updatePaymentRequest(id, { status: "cancelled" });
        return res.json({ paymentRequest: updated2, message: "Payment request cancelled" });
      }
      const recipientMatches = paymentRequest.toUserId === sessionUserId || !paymentRequest.toUserId && (paymentRequest.toEmail === (await storage.getUser(sessionUserId))?.email || paymentRequest.toPhone === (await storage.getUser(sessionUserId))?.phone);
      ...
      if (action === "decline") {
        const updated2 = await storage.updatePaymentRequest(id, { status: "declined" });
        return res.json({ paymentRequest: updated2, message: "Payment request declined" });
      }
    ...
    try {
      ...
      try {
        await applyLedgerEntry({
          ...
          entryType: "savings_contribution",
          idempotencyKey: `savings:${id}:${Date.now()}`,
  ...
  app2.post("/api/transfer", requireAuth, async (req, res) => {
    try {
      ...
      if (req.session.userId !== fromUserId) {
  ...
  app2.get("/api/transaction-fees", requireAuth, async (_req, res) => {
    try {
      res.json({
...
`;
  ...
  app2.get("/api/announcements", requireAuth, async (req, res) => {
    try {
      const userId = req.session.userId;
      const dismissed = await db.select({ announcementId: announcementDismissals.announcementId }).from(announcementDismissals).where(eq3(announcementDismissals.userId, userId));
      const dismissedIds = new Set(dismissed.map((item) => item.announcementId));
      const announcements2 = (await storage.getActiveAnnouncements()).filter((announcement) => !dismissedIds.has(announcement.id));
  ...
  app2.post("/api/announcements/:id/dismiss", requireAuth, async (req, res) => {
    try {
      const userId = req.session.userId;
      await db.insert(announcementDismissals).values({ userId, announcementId: req.params.id }).onConflictDoNothing();
      res.json({ success: true });
    } catch (error) {
  ...
  app2.post("/api/crypto/transfer", requireAuth, async (req, res) => {
    try {
      ...
      if (sourceType === "crypto" && destinationType === "crypto" && normalizeCurrency(sourceCoin) === normalizeCurrency(destinationCoin)) {
  ...
  app2.get("/api/deposit/nexuspay/status/:reference", requireAuth, async (req, res) => {
    try {
      const userId = req.session.userId;
      const { reference } = req.params;
      const [existingTransaction] = await db.select().from(transactions).where(eq3(transactions.reference, reference));
      if (existingTransaction && existingTransaction.userId !== userId) {
        return res.status(403).json({ message: "Transaction not found" });
      }
      const gateway = String(existingTransaction?.metadata?.gateway || "nexuspay");
      let status;
      if (gateway === "payhero") {
        ...
        status = {
          status: providerStatus.success && ["success", "completed", "paid"].includes(normalizedStatus) ? "completed" : ["failed", "cancelled", "rejected"].includes(normalizedStatus) ? "failed" : "pending",
          reference,
      ...
      if (status.status === "completed") {
        ...
        if (txn && txn.status !== "completed") {
          ...
          if (walletId) {
            const [wallet] = await db.select().from(wallets).where(eq3(wallets.id, walletId));
  ...
  app2.post("/api/exchange/swap", requireAuth, async (req, res) => {
    try {
      ...
      const security = await verifyTransactionSecurity(req.session.userId, { pin, authenticatorCode });
      if (!security.ok) return res.status(security.status || 400).json(security);
      ...
      try {
        ...
        await applyLedgerEntry({
          ...
          userId,
          currency: normalizeCurrency(toWallet.currency),
          amount: toAmount,
          entryType: "exchange",
          idempotencyKey: `exchange:${ref}:credit`,
